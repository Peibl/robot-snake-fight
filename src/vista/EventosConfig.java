...
public class EventosConfig {
	...
	public EventosConfig(PanelConfig datos){
		this.datos = datos;
		if (!(datos.tfNick.getText().matches("[a-zA-Z_0-9]*") && datos.tfNick.getText().length()>=4&&datos.tfNick.getText().length()<=10)){
			valido=false;
		}
		else{
			if (datos.tfIP.getText() == ""){
				...
				if (datos.tfPuerto.getText().length() == 4 || datos.tfPuerto.getText().length() == 5){
					try{
						//si no se puede castear implica que no es un puerto valido
